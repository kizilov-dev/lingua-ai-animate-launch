          ...
          {plans.map((plan, index) => (
            <div key={plan.name}
                 className={`flex flex-col rounded-3xl shadow-xl p-8 items-center bg-white 
                              transition-transform hover:-translate-y-2 border-2 ${index === 1 ? "border-violet-500 scale-105 shadow-2xl z-10" : "border-transparent" }
                              animate-fade-in-up`}
              ...
              <a href="#cta"
                 className={`mt-6 w-full py-3 rounded-xl text-center font-semibold text-lg transition-all duration-200 
                   ${index === 1 ? "bg-gradient-to-tr from-indigo-600 via-purple-600 to-violet-500 text-white hover:brightness-105 shadow-md hover:shadow-xl" 
